...
| Data Health | `.admin-health`, `__hint`, `__hint-row`, `__after`, `__list`, `__grid--2`, `__grid--3`, `__server`, `__settings`, `__schedule` | `admin.css` | `admin/data_health`: each section is a `.list-section` head, one hint saying what the numbers mean, then a `.stat-panel`. The grid modifiers exist because `.stat-panel__grid`'s hairlines assume four cells. No filled button; a failed job's Discard is `.btn--plain` with a confirm, not a red outline repeated down the list |
